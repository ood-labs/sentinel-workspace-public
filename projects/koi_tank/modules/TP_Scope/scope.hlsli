// TP_Scope / scope.hlsli — shared declarations for the exploded instrument view.
//
// TP_Scope is the only node in this show that renders NOTHING of its own. Every mark it draws
// stands for a number some other node already published: TP_Plan's tank, TP_Sim's field,
// TP_Caustics' atlas, TP_School's records. That is the whole discipline of the thing — if a line
// in here is not a measurement, it should not be drawn.
//
// It sits between TP_Render and TP_Post on purpose. TP_Render publishes LINEAR RAY DISTANCE in
// alpha, and TP_Post ignores alpha entirely, so this node is the one place in the chain that can
// depth-test 3D overlay geometry against the real scene. Downstream of TP_Post it would be a
// sticker on a finished frame; upstream of it, the overlay also collects the bloom and the grade,
// which is most of what stops it looking like a wireframe.
