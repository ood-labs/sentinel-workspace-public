// VC_Post / blur_v.hlsl — vertical half of the separable bloom gaussian.
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float step = bloom_radius / res.y;

    float3 s = float3(0, 0, 0);
    [unroll]
    for (int i = 0; i < 9; i++)
        s += _Tex0.SampleLevel(LinearSampler, uv + float2(0.0, ((float)i - 4.0) * step), 0).rgb * W9[i];
