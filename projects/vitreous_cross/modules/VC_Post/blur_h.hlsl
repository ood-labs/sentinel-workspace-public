// VC_Post / blur_h.hlsl — horizontal half of the separable bloom gaussian.
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float step = bloom_radius / res.x;

    float3 s = float3(0, 0, 0);
    [unroll]
    for (int i = 0; i < 9; i++)
        s += _Tex0.SampleLevel(LinearSampler, uv + float2(((float)i - 4.0) * step, 0.0), 0).rgb * W9[i];
