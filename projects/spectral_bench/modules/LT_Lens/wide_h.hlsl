// LT_Lens / blur_h.hlsl — horizontal half of the wide halation octave, four times coarser.
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float st = halation_radius / res.x;
    float3 s = 0.0.xxx;
    [unroll] for (int i = 0; i < 9; i++)
        s += _Tex0.SampleLevel(LinearSampler, uv + float2(((float)i - 4.0) * st, 0.0), 0).rgb * W9[i];
