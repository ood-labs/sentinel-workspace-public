// Passive preview for the Topographic signal/control bus.
//
// This node publishes scalar authority; exact editing belongs in Properties.
// The texture is intentionally a small, non-interactive diagnostic preview.
...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    col += T.ink * sui3Text(P, float2(18.0, 18.0), 3.0,
        S_S,S_I,S_G,S_N,S_A,S_L,S_SP,S_B,S_U,S_S,0,0);
    ...
    [loop] for (int lane = 0; lane < 4; ++lane) {
        float value = saturate(laneValue(lane, d));
    ...
    col += T.accent * sui3Fixed(P, float2(R.x - 64.0, 237.0), 1.0,
                                d.energy, 2);
