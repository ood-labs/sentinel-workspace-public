...
StructuredBuffer<LabelOverride> _Tex0 : register(t0);
RWStructuredBuffer<LabelEditState> OutputBuffer : register(u0);
...
{
    LabelEditState state = OutputBuffer[0];
    state.command = 0.0;
    ...
    [loop] for (uint i = 0u; i < events; ++i) {
        ...
        if (e.phase == 5u && _ViewportSelectionMeta.y >= 1001u && _ViewportSelectionMeta.y <= 1012u) {
            uint objectId = _ViewportSelectionMeta.y;
            LabelOverride edit = _Tex0[objectId - 1001u];
