...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    SaLayout L = saLayout(R, t0.x, t0.y, t0.z, t1.x, t1.y, t1.z, t1.w);
    ...
    [loop] for (int gi = 0; gi < 8; ++gi) {
        ...
        // Every one of the eight specimen cells was drawing itself for every
        // pixel on the sheet -- graticules, ticks, reticles and registration
        // marks evaluated eight times over, seven of them guaranteed to contribute
        // nothing. Nothing in a cell draws outside its own rect, so one bounding
        // reject collapses eight cell evaluations to one.
        if (P.x < rg.x - 2.0 || P.x > rg.z + 2.0 ||
            P.y < rg.y - 2.0 || P.y > rg.w + 2.0) continue;
