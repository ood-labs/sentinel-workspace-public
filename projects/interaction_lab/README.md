...
This is a foundation rather than a full DCC toolset. It does not yet include snapping, numeric transform entry, spline segment insertion, depth-tested gizmo fading, or host-mirrored sub-object selection. Those can be layered on as additional Module passes and controls without changing the application. The untracked `captures/` and `proof/ui-kit-pulse/` directories are local stress artifacts; the compact final proof bundles remain part of the project.
