...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    [loop] for (uint g = start; g <= latest; ++g) {
        ...
        // Adaptive measures against the band's OWN rolling level, which makes the
        // threshold immune to input gain but also means a breakdown drags the
        // reference down under the line and quiet material starts firing. Fixed
        // measures against a constant instead, so the threshold becomes a genuine
        // absolute level: the lane fires when Edb > fixed_ref_db + threshold and
        // a quiet passage simply stays under it.
        float reference = (adapt_mode == 1) ? fixed_ref_db : base;
        float flux = max(0.0, Edb - reference);

        // The rolling baseline keeps tracking in BOTH modes. It costs one lerp and
        // it means switching back to Adaptive resumes from the band's real current
        // level rather than a value frozen whenever the mode last changed.
