> **Vendored into formicary, unmodified apart from the module name.**
>
> Copied from `projects/cloth_lab/modules/cloth_bands` at build 0.5.57, together with its one
> dependency, `_shared/au_hud/au_text.hlsli`. The workspace rule is that a show bundles its own
> copy of the audio analyser rather than linking to Cloth Lab at runtime, so this is a fork by
> design and not a shortcut: it is generic DSP infrastructure with no creative identity of its
> own, and formicary owns this copy from here.
>
> It was NOT reimplemented. The detection is subtle — per-lane dB thresholds measured against a
> rolling baseline, refractory gating, and a ring catch-up contract against the audio node's
> generation counter — and rewriting all of that to drive one parameter would have been a way to
> introduce bugs that cloth_lab has already found and fixed. What formicary adds is downstream:
> see the project README for how `kick` drives the colony's walking speed.
>
> Its Canvas panel is the tuning surface. Tune thresholds there by dragging, not by guessing.
