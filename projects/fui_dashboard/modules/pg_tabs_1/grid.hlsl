...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    if ((int)i < N)
    {
        ...
        else if (mode == 4) {                              // Line (explicit start -> end)
            float t = (N > 1) ? (float)i/(float)(N-1) : 0.0;
            p.pos = lerp(line_start, line_end, t) + (h22(fi)-0.5)*jitter;
            p.dir = normalize(line_end - line_start + 1e-4);
            p.u = t; p.group = 0; p.active = 1;
        }
