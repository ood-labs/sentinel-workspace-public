...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    uint c0 = min((uint)_Data0_Count, 256u);
    [loop]
    for (uint i = 0u; i < 256u; i++) { if (i >= c0) break; accum(_Data0[i], uv, camDrift, col, aAcc); }

    uint c1 = min((uint)_Data1_Count, 256u);
    [loop]
    for (uint j = 0u; j < 256u; j++) { if (j >= c1) break; accum(_Data1[j], uv, camDrift, col, aAcc); }

    uint c2 = min((uint)_Data2_Count, 256u);
    [loop]
    for (uint k = 0u; k < 256u; k++) { if (k >= c2) break; accum(_Data2[k], uv, camDrift, col, aAcc); }

    uint c3 = min((uint)_Data3_Count, 256u);
    [loop]
    for (uint m = 0u; m < 256u; m++) { if (m >= c3) break; accum(_Data3[m], uv, camDrift, col, aAcc); }

    uint c4 = min((uint)_Data4_Count, 256u);
    [loop]
    for (uint a = 0u; a < 256u; a++) { if (a >= c4) break; accum(_Data4[a], uv, camDrift, col, aAcc); }

    uint c5 = min((uint)_Data5_Count, 256u);
    [loop]
    for (uint b = 0u; b < 256u; b++) { if (b >= c5) break; accum(_Data5[b], uv, camDrift, col, aAcc); }
