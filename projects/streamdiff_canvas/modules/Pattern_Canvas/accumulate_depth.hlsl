...
StructuredBuffer<StampPose> ActiveStampPose : register(t4);
...
float2 pcDepthFeedbackSample(float2 uv)
{
    ...
    float4 depthAndMask = _Tex2.SampleLevel(
        LinearSampler, saturate(sampleUv), 0);
    float2 result = depthAndMask.ra;
    if (edgeMode == 0) result *= inside;
    return result;
}

float2 pcDepthStamp(float2 uv, int revealStage)
{
    StampPose pose = ActiveStampPose[0];
    if (pose.visible == 0u) return 0.0.xx;
    ...
    return float2(shapedDepth * coverage, coverage);
}
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float4 previousSample = _Tex2.SampleLevel(LinearSampler, uv, 0);
    float2 previousDepthAndMask = previousSample.ra;

    if (action < -0.5) {
        previousDepthAndMask = 0.0.xx;
    }
    else {
        if (feedback_enabled != 0) {
            previousDepthAndMask = pcDepthFeedbackSample(uv);
        }
        float dt = clamp(_DeltaTime, 0.0, 0.1);
        previousDepthAndMask *= exp(-max(depth_fade, 0.0) * dt);
    }

    if (action > 0.5) {
        int revealStage = (int)round(clamp(action, 1.0, 3.0));
        float2 stampDepthAndMask = pcDepthStamp(uv, revealStage);
        previousDepthAndMask = max(previousDepthAndMask, stampDepthAndMask);
    }

    float coverage = saturate(previousDepthAndMask.y);
    float depth = min(saturate(previousDepthAndMask.x), coverage);
    OutputUAV[pixel] = float4(depth, depth, depth, coverage);
}
