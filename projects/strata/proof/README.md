...
- `feature-analysis-proxy.png` is the current 320 x 480 portrait analysis
  texture between `plate_comp` and `Features #0`.
...
- The downscaled branch reported 15 live corners with coordinates bounded by
  320 x 480. `corner_thread` consumes the proxy as an extent reference and
  rescales those records across its full 720 x 1080 output.
