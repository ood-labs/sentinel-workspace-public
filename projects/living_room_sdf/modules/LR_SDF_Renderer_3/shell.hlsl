...
float2 archObject(float3 p,float k,float3 d,uint i)
{
    ...
    if(k<3.5){float2 wall=H(bx(p-float3(0,h*.5,0),d*.5),(i==2||i==12)?2:1);if(i==12){[loop]for(int j=-5;j<=5;j++)wall=U(wall,H(rb(p-float3(j*.34,h*.50,z*.72),float3(.022,h*.43,.028),.008),9));}return wall;}
    if(k<4.5){
...
{
    float sh=1;
    [loop]for(uint i=0;i<min(_Data1_Count,23);i++){float2 q=p.xz-float2(_Data1[i].position[0],_Data1[i].position[2])-float2(.10,-.16);q=r2(q,-_Data1[i].yaw);float2 d=float2(_Data1[i].width,_Data1[i].depth)*.62+.16;float ellipse=length(q/max(d,.08));sh*=lerp(.20,1,smoothstep(.42,1.45,ellipse));}
    return sh;
}
float3 shade(float3 p,float3 n,float3 v,float t,uint mid){Mat m=mat(mid);float3 base=baseColor(m,p,t);float ao=aoAt(p,n);if(mid==0||mid==6)ao*=contactShadow(p);n=bumpNormal(m,p,n,t);float3 col=base*(.07+.15*saturate(n.y*.5+.5))*ao;
    [loop]for(uint i=0;i<min(_Data3_Count,6);i++){if(_Data3[i].type_id>2.5){col+=base*lc(i)*_Data3[i].intensity*.24;continue;}float3 dl=lp(i)-p;float dist=length(dl),att=pow(saturate(1-dist/max(_Data3[i].range,.1)),2),ndl=saturate(dot(n,dl/max(dist,.001)));float3 l=dl/max(dist,.001),hh=normalize(l+v);float fres=pow(1-saturate(dot(v,hh)),5);float specAmp=m.spec*lerp(.08,1,m.metal)*pow(1-m.rough*.72,2);float3 specular=lerp(.04.xxx,base,m.metal)*(specAmp*pow(saturate(dot(n,hh)),lerp(96,6,m.rough))*(1+fres));col+=(base*(1-m.metal)*ndl+specular)*lc(i)*_Data3[i].intensity*att;}return col+base*m.emit;}
