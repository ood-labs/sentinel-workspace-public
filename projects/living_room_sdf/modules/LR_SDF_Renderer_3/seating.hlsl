...
float2 seatObject(float3 p,float k,float3 d,uint i)
{
    ...
    if(k<6.5){
        v=H(rb(p-float3(0,.30,z*.03),float3(w*.46,.20,z*.43),.055),7);v=U(v,H(rb(p-float3(0,.74,-z*.39),float3(w*.46,.31,z*.095),.045),7));
        v=U(v,H(rb(p-float3(-w*.46,.53,0),float3(w*.065,.38,z*.45),.038),7));v=U(v,H(rb(p-float3(w*.46,.53,0),float3(w*.065,.38,z*.45),.038),7));
        [loop]for(int j=-1;j<=1;j++){v=U(v,H(rb(p-float3(j*w*.29,.58,z*.035),float3(w*.135,.125,z*.335),.032),7));v=U(v,H(cp(p,float3(j*w*.29-w*.11,.695,z*.365),float3(j*w*.29+w*.11,.695,z*.365),.010),7));}
    ...
    if(k<7.5){
        v=H(rb(p-float3(0,.31,0),float3(w*.43,.22,z*.42),.052),8);v=U(v,H(rb(p-float3(0,.72,-z*.38),float3(w*.40,.32,z*.095),.042),8));
        v=U(v,H(rb(p-float3(-w*.43,.52,0),float3(w*.085,.35,z*.42),.036),8));v=U(v,H(rb(p-float3(w*.43,.52,0),float3(w*.085,.35,z*.42),.036),8));v=U(v,H(cp(p,float3(-w*.30,.66,z*.38),float3(w*.30,.66,z*.38),.012),8));
...
float3 shade(float3 p,float3 n,float3 v,float t,uint mid){Mat m=mat(mid);float3 base=baseColor(m,p,t);float ao=aoAt(p,n);n=bumpNormal(m,p,n,t);float3 col=base*(.07+.15*saturate(n.y*.5+.5))*ao;[loop]for(uint i=0;i<min(_Data3_Count,6);i++){if(_Data3[i].type_id>2.5){col+=base*lc(i)*_Data3[i].intensity*.24;continue;}float3 dl=lp(i)-p;float dist=length(dl),att=pow(saturate(1-dist/max(_Data3[i].range,.1)),2),ndl=saturate(dot(n,dl/max(dist,.001)));float3 l=dl/max(dist,.001),hh=normalize(l+v);float fres=pow(1-saturate(dot(v,hh)),5);float specAmp=m.spec*lerp(.08,1,m.metal)*pow(1-m.rough*.72,2);float3 specular=lerp(.04.xxx,base,m.metal)*(specAmp*pow(saturate(dot(n,hh)),lerp(96,6,m.rough))*(1+fres));col+=(base*(1-m.metal)*ndl+specular)*lc(i)*_Data3[i].intensity*att;}col*=lerp(.68,1,saturate((p.y-.10)/.30));return col+base*m.emit;}
