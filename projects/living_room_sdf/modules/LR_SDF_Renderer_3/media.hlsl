...
float2 mediaObject(float3 p,float k,float3 d,uint i)
{
    ...
    if(k<16.5){
        v=H(cy(p-float3(0,.24,0),.24,w*.32),14);[loop]for(int j=0;j<11;j++){float a=j*.571;float tier=(float)(j%3);float3 a1=float3(cos(a)*w*(.13+.025*tier),h*(.58+.055*tier+.018*(j&1)),sin(a)*z*(.13+.025*tier));v=U(v,H(cp(p,float3(0,.43,0),a1,.014),16));float3 q=p-(a1+float3(cos(a)*w*.14,h*(.055+.018*tier),sin(a)*z*.14));q.xz=r2(q.xz,-a);v=U(v,H(el(q,float3(w*.19,h*.065,z*.11)),15));}return v;
    }
    if(k<18.5){
