...
float2 objectPivot(uint objectId) {
    ...
    [loop] for (uint i = 0u; i < LR_RECORD_COUNT; ++i) if (lrObjectForRecord(i) == objectId) { pivot += _Tex0[i].position.xz; count += 1.0; }
...
void main(uint3 tid : SV_DispatchThreadID) {
    ...
    PNode r = _Tex0[i]; uint objectId = lrObjectForRecord(i); FurnishingState state = _Tex1[objectId - 1u];
    float2 pivot = objectPivot(objectId);
    r.position.xz = pivot + lrRotate(r.position.xz - pivot, state.yaw_offset) + state.offset;
    r.yaw += state.yaw_offset; r.dir = float2(-sin(r.yaw), cos(r.yaw)); OutputBuffer[i] = r;
}
