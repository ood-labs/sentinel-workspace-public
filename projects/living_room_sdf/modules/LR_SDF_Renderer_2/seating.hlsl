...
float2 seatObject(float3 p,float k,float3 d,uint i)
{
    ...
    if(k<6.5){
        v=H(rb(p-float3(0,.30,z*.03),float3(w*.46,.20,z*.43),.10),7);v=U(v,H(rb(p-float3(0,.74,-z*.39),float3(w*.46,.31,z*.095),.08),7));
        v=U(v,H(rb(p-float3(-w*.46,.53,0),float3(w*.065,.38,z*.45),.07),7));v=U(v,H(rb(p-float3(w*.46,.53,0),float3(w*.065,.38,z*.45),.07),7));
        [loop]for(int j=-1;j<=1;j++)v=U(v,H(rb(p-float3(j*w*.29,.58,z*.035),float3(w*.135,.125,z*.335),.06),7));
        [loop]for(int f=-1;f<=1;f+=2){v=U(v,H(cy(p-float3(f*w*.38,.08,z*.31),.07,.032),10));v=U(v,H(cy(p-float3(f*w*.38,.08,-z*.31),.07,.032),10));}return v;
    }
    if(k<7.5){
        v=H(rb(p-float3(0,.31,0),float3(w*.43,.22,z*.42),.10),8);v=U(v,H(rb(p-float3(0,.72,-z*.38),float3(w*.40,.32,z*.095),.08),8));
        v=U(v,H(rb(p-float3(-w*.43,.52,0),float3(w*.085,.35,z*.42),.07),8));v=U(v,H(rb(p-float3(w*.43,.52,0),float3(w*.085,.35,z*.42),.07),8));
        [loop]for(int f=-1;f<=1;f+=2){v=U(v,H(cy(p-float3(f*w*.33,.08,z*.30),.08,.035),10));v=U(v,H(cy(p-float3(f*w*.33,.08,-z*.30),.08,.035),10));}return v;
    }
    if(k<8.5){
        v=H(rb(p-float3(0,h-.06,0),float3(w*.5,.06,z*.5),.05),9);[loop]for(int x=-1;x<=1;x+=2)[loop]for(int q=-1;q<=1;q+=2)v=U(v,H(cy(p-float3(x*w*.39,h*.43,q*z*.36),h*.42,.032),10));
    ...
    if(k<19.5)return H(rb(p-float3(0,h*.5,0),d*.5,.11),8);
    if(k<20.5)return H(rb(p-float3(0,h*.5,0),d*.5,.08),19);
...
float aoAt(float3 p,float3 n){float o=0,s=1;[loop]for(int j=1;j<=4;j++){float h=.04+j*.07;o+=(h-mapScene(p+n*h).x)*s;s*=.68;}return saturate(1-o*ao_strength);}
...
float3 baseColor(Mat m,float3 p,float t){float lod=1/(1+t*m.scale*.018),f=.5+.5*sin(dot(p,float3(7.1,3.3,5.7))+m.seed);if(m.pattern<1.5)f=.5+.35*sin((p.x*10+p.z*1.3+f*1.2)*lod);else if(m.pattern>4.5&&m.pattern<8.5)f=.45+.25*sin(p.x*m.scale)*sin((p.y+p.z)*m.scale*.8)*lod;return lerp(m.a,m.b,saturate(f)*m.amount);}
...
float3 shade(float3 p,float3 n,float3 v,float t,uint mid){Mat m=mat(mid);float3 base=baseColor(m,p,t),col=base*(.06+.13*saturate(n.y))*aoAt(p,n);[loop]for(uint i=0;i<min(_Data3_Count,6);i++){if(_Data3[i].type_id>2.5){col+=base*lc(i)*_Data3[i].intensity*.22;continue;}float3 dl=lp(i)-p;float dist=length(dl),att=pow(saturate(1-dist/max(_Data3[i].range,.1)),2),ndl=saturate(dot(n,dl/max(dist,.001)));float3 hh=normalize(dl/max(dist,.001)+v);col+=(base*ndl+pow(saturate(dot(n,hh)),lerp(80,5,m.rough))*m.spec)*lc(i)*_Data3[i].intensity*att;}return col+base*m.emit;}
