...
float2 seatObject(float3 p,float k,float3 d,uint i)
{
    ...
    if(k<6.5){
        ...
        v=U(v,H(rb(p-float3(-w*.455,.43,0),float3(w*.050,.245,z*.405),.040),7));v=U(v,H(rb(p-float3(w*.455,.43,0),float3(w*.050,.245,z*.405),.040),7));
        [loop]for(int j=-1;j<=1;j++){float3 cc=float3(j*w*.29,.45,z*.035);float cushion=rb(p-cc,float3(w*.132,.105,z*.325),.045);cushion=max(cushion,-sp(p-(cc+float3(0,.112,z*.025)),.018));v=U(v,H(cushion,7));v=U(v,H(cp(p,float3(j*w*.29-w*.105,.545,z*.352),float3(j*w*.29+w*.105,.545,z*.352),.007),11));v=U(v,H(cp(p,cc+float3(-w*.125,.09,-z*.27),cc+float3(-w*.125,.09,z*.28),.006),11));v=U(v,H(cp(p,cc+float3(w*.125,.09,-z*.27),cc+float3(w*.125,.09,z*.28),.006),11));float3 bc=float3(j*w*.29,.72,-z*.315),bq=p-bc;bq.yz=r2(bq.yz,-.10+j*.025);v=U(v,H(rb(bq,float3(w*.130,.205,z*.072),.045),7));v=U(v,H(cp(p,bc+float3(-w*.102,.198,z*.073),bc+float3(w*.102,.198,z*.073),.006),11));}
    ...
    if(k<7.5){
        ...
        v=U(v,H(rb(p-float3(-w*.425,.43,0),float3(w*.055,.245,z*.37),.040),8));v=U(v,H(rb(p-float3(w*.425,.43,0),float3(w*.055,.245,z*.37),.040),8));float3 sc=float3(0,.445,z*.035);float seat=rb(p-sc,float3(w*.31,.105,z*.30),.045);seat=max(seat,-sp(p-(sc+float3(0,.112,z*.025)),.017));v=U(v,H(seat,8));v=U(v,H(cp(p,sc+float3(-w*.295,.09,-z*.24),sc+float3(-w*.295,.09,z*.25),.007),9));v=U(v,H(cp(p,sc+float3(w*.295,.09,-z*.24),sc+float3(w*.295,.09,z*.25),.007),9));float3 bc=float3(0,.71,-z*.305),bq=p-bc;bq.yz=r2(bq.yz,-.13);v=U(v,H(rb(bq,float3(w*.29,.205,z*.068),.045),8));v=U(v,H(cp(p,float3(-w*.275,.54,z*.338),float3(w*.275,.54,z*.338),.008),9));
    ...
    if(k<8.5){
        v=H(rb(p-float3(0,h-.06,0),float3(w*.5,.06,z*.5),.050),9);[loop]for(int x=-1;x<=1;x+=2)[loop]for(int q=-1;q<=1;q+=2)v=U(v,H(cy(p-float3(x*w*.39,h*.43,q*z*.36),h*.42,.032),10));
        v=U(v,H(rb(p-float3(0,h*.28,0),float3(w*.38,.025,z*.30),.012),9));return U(v,H(bx(p-float3(0,h*.62,0),float3(w*.40,.025,z*.34)),9));
    }
    ...
    if(k<14.5){
        ...
        v=U(v,H(max(cy(p-float3(w*.45,h*.82,0),.19,w*.34),-cy(p-float3(w*.45,h*.82,0),.20,w*.25)),20));v=U(v,H(cy(p-float3(w*.45,h*.63,0),.010,w*.34),13));v=U(v,H(cy(p-float3(w*.45,h*1.01,0),.010,w*.29),13));return U(v,H(sp(p-float3(w*.45,h*.78,0),.07),21));
    }
    if(k<15.5){
        v=H(rb(p-float3(0,h*.18,0),float3(w*.27,h*.18,w*.27),.08),12);v=U(v,H(cy(p-float3(0,h*.46,0),h*.13,.025),13));
        v=U(v,H(max(cy(p-float3(0,h*.76,0),h*.20,w*.46),-cy(p-float3(0,h*.76,0),h*.22,w*.34)),20));v=U(v,H(cy(p-float3(0,h*.56,0),.010,w*.45),13));v=U(v,H(cy(p-float3(0,h*.96,0),.010,w*.36),13));return U(v,H(sp(p-float3(0,h*.68,0),.06),21));
    }
    if(k<19.5){v=H(rb(p-float3(0,h*.56,0),float3(w*.48,h*.31,z*.46),.065),8);v=U(v,H(rb(p-float3(0,h*.18,0),float3(w*.43,h*.10,z*.40),.028),9));[loop]for(int x=-1;x<=1;x+=2)[loop]for(int q=-1;q<=1;q+=2){v=U(v,H(cy(p-float3(x*w*.35,.075,q*z*.32),.065,.024),9));v=U(v,H(cy(p-float3(x*w*.35,.025,q*z*.32),.022,.038),13));}return v;}
    if(k<20.5){float pillow=rb(p-float3(0,h*.5,0),d*.5,.040);pillow=max(pillow,-sp(p-float3(0,h*.53,z*.48),.018));v=H(pillow,19);v=U(v,H(cp(p,float3(-w*.40,h*.94,z*.48),float3(w*.40,h*.94,z*.48),.006),11));return v;}
    p.y+=.060;v=H(rb(p-float3(-w*.15,.03,0),float3(w*.48,.025,z*.45),.015),23);v=U(v,H(rb(p-float3(-w*.10,.085,0),float3(w*.42,.022,z*.42),.010),22));v=U(v,H(rb(p-float3(-w*.07,.115,0),float3(w*.38,.009,z*.38),.005),20));v=U(v,H(rb(p-float3(-w*.02,.145,-z*.02),float3(w*.34,.020,z*.34),.008),17));
    float3 cup=float3(w*.34,.15,0);v=U(v,H(cy(p-cup,.085,w*.22),12));v=U(v,H(max(cy(p-(cup+float3(0,.085,0)),.010,w*.29),-cy(p-(cup+float3(0,.085,0)),.018,w*.18)),12));v=U(v,H(cy(p-float3(w*.34,.045,0),.010,w*.38),12));float3 handle=float3(w*.55,.15,0),hq=p-handle;v=U(v,H(max(cy(hq.xzy,.018,w*.19),-cy(hq.xzy,.025,w*.11)),12));v=U(v,H(cp(p,float3(-w*.38,.17,z*.30),float3(w*.04,.17,z*.13),.009),13));return v;
}

float3 mapScene(float3 p){p=lrDomainDistort(p,fx_seating);float3 best=float3(1000,-1,-1);[loop]for(uint i=0;i<min(_Data1_Count,23);i++){if(!active(i))continue;float3 pos=fp(i),d=max(fd(i),.01),q=p-pos;q.xz=r2(q.xz,-_Data1[i].yaw);float2 s=seatObject(q,_Data1[i].kind_id,d,i);if(s.x<best.x)best=float3(s.x,s.y,(float)i);}best.x*=lrDistortLip(fx_seating);return best;}
...
float3 shade(float3 p,float3 n,float3 v,float t,uint mid)
{
    ...
    float soft=pow(saturate(dot(n,normalize(float3(-.38,.72,.42)))),mid==8?7:18);if(mid==8)col+=lerp(base,1.0.xxx,.28)*soft*.16;if(mid==7)col+=base*soft*.055;if(mid==9)col+=float3(.16,.09,.04)*soft*.13;if(mid==13)col+=float3(.55,.32,.10)*soft*.22;
