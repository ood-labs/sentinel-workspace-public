...
void main(uint3 id : SV_DispatchThreadID) {
    ...
    if (seating) { float2 center = float2(-0.45,1.20) + layout_offset; r.position.xz = center + (r.position.xz - center) * seating_spread; }
    ...
    if (i == 14u || i == 15u) { float plantCenterX = -0.40 + layout_offset.x; r.position.x = plantCenterX + (r.position.x - plantCenterX) * plant_spread; r.position.z += plant_depth_offset; }
