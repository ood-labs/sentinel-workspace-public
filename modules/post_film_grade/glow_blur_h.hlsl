...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    sum += _Tex0.SampleLevel(LinearSampler, uv + float2(texel.x * stride * 1.0, 0.0), 0).rgb * 0.17603266;
    sum += _Tex0.SampleLevel(LinearSampler, uv - float2(texel.x * stride * 1.0, 0.0), 0).rgb * 0.17603266;
    sum += _Tex0.SampleLevel(LinearSampler, uv + float2(texel.x * stride * 2.0, 0.0), 0).rgb * 0.12098177;
    sum += _Tex0.SampleLevel(LinearSampler, uv - float2(texel.x * stride * 2.0, 0.0), 0).rgb * 0.12098177;
    sum += _Tex0.SampleLevel(LinearSampler, uv + float2(texel.x * stride * 3.0, 0.0), 0).rgb * 0.06475880;
    sum += _Tex0.SampleLevel(LinearSampler, uv - float2(texel.x * stride * 3.0, 0.0), 0).rgb * 0.06475880;
    sum += _Tex0.SampleLevel(LinearSampler, uv + float2(texel.x * stride * 4.0, 0.0), 0).rgb * 0.02839769;
    sum += _Tex0.SampleLevel(LinearSampler, uv - float2(texel.x * stride * 4.0, 0.0), 0).rgb * 0.02839769;
