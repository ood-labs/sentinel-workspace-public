...
// Visual intent: TouchDesigner-inspired neon data instrument: black field, cyan/magenta/amber blob halos, measured bounds, line-network energy, scientific HUD
...
void main(uint3 DTid : SV_DispatchThreadID) {
    ...
    for (uint i = 0; i < count; ++i) {
        float2 q = (float2(normCoord(_Data0[i].centroidX, _Resolution.x), normCoord(_Data0[i].centroidY, _Resolution.y)) - 0.5) * float2(aspect, 1.0);
        float conf = saturate(_Data0[i].area);
