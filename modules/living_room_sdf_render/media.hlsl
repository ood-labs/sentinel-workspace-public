...
float2 mediaObject(float3 p,float k,float3 d,uint i)
{
    ...
    if(k<10.5){
        v=H(rb(p-float3(0,h*.60,0),float3(w*.5,h*.38,z*.5),.035),9);v=U(v,H(bx(p-float3(0,h*.14,0),float3(w*.46,h*.055,z*.42)),10));
    ...
    if(k<11.5){
        v=H(rb(p-float3(0,h*.5,0),float3(w*.5,h*.5,z*.50),.035),10);v=U(v,H(rb(p-float3(0,h*.52,-z*.56),float3(w*.465,h*.44,.012),.015),10));
        v=U(v,H(bx(p-float3(0,h*.50,z*.70),float3(w*.22,h*.18,z*.18)),3));v=U(v,H(cy(p-float3(0,-.12,z*.65),.12,.022),3));return U(v,H(sp(p-float3(w*.39,h*.08,-z*.59),.012),21));
    }
    if(k<12.5){
    ...
    if(k<16.5){
        v=H(cy(p-float3(0,.24,0),.24,w*.32),14);[loop]for(int j=0;j<7;j++){float a=j*.897;float3 a1=float3(cos(a)*w*.18,h*(.64+.035*(j&1)),sin(a)*z*.18);v=U(v,H(cp(p,float3(0,.45,0),a1,.018),16));float3 q=p-(a1+float3(cos(a)*w*.16,h*.08,sin(a)*z*.16));q.xz=r2(q.xz,-a);v=U(v,H(el(q,float3(w*.27,h*.10,z*.16)),15));}return v;
    }
    if(k<18.5){
...
float3 mapScene(float3 p){float3 best=float3(1000,-1,-1);[loop]for(uint i=0;i<min(_Data1_Count,23);i++){if(!active(i))continue;float3 pos=fp(i),d=max(fd(i),.01),q=p-pos;q.xz=r2(q.xz,-_Data1[i].yaw);float2 s=mediaObject(q,_Data1[i].kind_id,d,i);if(s.x<best.x)best=float3(s.x,s.y,(float)i);}return best;}
