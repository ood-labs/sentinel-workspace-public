...
Use `sentinel_vision action=eval_pipeline pipeline_id=<id> preset=render_quality` for one-call AI visual review of a live pipeline. It captures `<workspace>/captures/vision_<timestamp>/output.png`, evaluates it through the configured OpenAI-compatible provider, and returns `_meta.captured_png`.

For first-time setup, run `sentinel_vision action=status` and open the exact returned `config_path`. `vision.json` contains a top-level `providers` array: put an OpenRouter key only in the `openrouter` profile and a Google AI Studio Gemini key only in a separate `gemini` profile. OpenRouter uses model ids such as `google/gemini-3.5-flash`; direct Gemini uses the OpenAI-compatible endpoint and a model such as `gemini-3-flash-preview`. Set `default_provider` to select the implicit route. Environment setup is also supported with `SENTINEL_VISION_API_KEY` or `OPENROUTER_API_KEY` set before launching Codex/MCP.

Validate every configured route with `status provider=<name>` until `key_present` and `key_ok` are true and `model_count` is nonzero, then run a real image evaluation; authentication alone is not multimodal proof. Never ask the user to paste API keys into chat, pass them as tool arguments, print them during inspection, or commit `vision.json`. `sentinel_vision action=configure` edits provider metadata only. See `knowledge/vision-eval.md` for exact provider blocks, safe key placement, model verification, and troubleshooting.
