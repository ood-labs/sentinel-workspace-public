...
Binds and expressions interact under one rule: a bind network may carry at most one expression-driven endpoint, and that expression drives the whole network. Setting a second expression anywhere on the same network is rejected. Dragging a bound parameter never removes the bind; dragging an expression-driven parameter still clears that expression (override on touch).

Scene Group exposed parameters are binds. The group parameter and member parameter form a two-endpoint network (one per component for color and XY compounds), so the member's own slider and the group widget move together and the link survives touching either side. Projects saved before binds existed, where exposes were one-way `ref()` expressions, migrate automatically on load.
