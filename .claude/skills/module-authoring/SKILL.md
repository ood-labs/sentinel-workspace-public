...
## Platform Traps (cost real time in shipped builds)

- **No `fwidth` / `ddx` / `ddy` in compute passes.** Screen-space derivatives are a pixel-shader feature. A compute pass that needs constant-weight lines under a nonlinear mapping (perspective grid, polar field, receding plane) must derive the width from the **analytic gradient** of that mapping. For a floor plane `z = 1/(y - horizon)`, `wx = x*z`: `|grad wx| = sqrt(z² + wx²/(y-horizon)²)`, then compare the distance-to-nearest-line against `grad * pixelSize`. Falling back to a fixed width aliases receding lines out entirely.
- **Persistent buffers only rebuild when their signature changes — and shader edits are not in the signature.** A generator that regenerates on `hash(parameters)` will keep serving the previously generated result after you rewrite its algorithm, with a successful compile and no error. Put an explicit `#define PLAN_VERSION n` into the signature and bump it whenever the algorithm changes. This failure is silent and looks exactly like "my edit did nothing".
- **Draw passes leave untouched pixels at alpha 0.** The rasterized target is not cleared to opaque black, so the module's own preview reads as "white background" in any viewer that composites over white. Resolve it explicitly in a following compute pass (`col = src.rgb * src.a`) rather than leaving every downstream consumer to guess.
- **Coverage needs its own output when colour cannot carry it.** An opaque black shape on a black background is undecidable from the colour lane. Add a second output pass publishing `src.a`; do not put coverage in the colour lane's alpha, which makes the preview read wrong.
- **Choose draw passes over SDF raymarching past ~100 primitives.** A few hundred spheres plus tubes is ~600 primitives; evaluating that per march step per pixel is hopeless, while vertex-pulled real geometry with the shared depth buffer is trivial. 320 nodes × (960 sphere + 96 tube) = 337,920 vertices runs comfortably. Set `vertex_count` to the maximum and emit degenerate triangles (`Position = float4(0,0,-999,1)`) for inactive records.
- **Derive mesh/UV parameters from generated vertex attributes, never from `atan2` of the normal.** Computing longitude as `atan2(n.z, n.x)` in the pixel shader puts a seam down every sphere where the derivative used for line width explodes. Generate `(u,v)` in the vertex shader from the slice/stack indices and interpolate them — continuous across the wrap, and exact.
- **Engine-injected names must not be redeclared.** A pass input binds `_Tex0`/`_Tex1`/... and `LinearSampler` automatically; declaring them yourself is `X3003: redefinition`. Same for `_DataN`, `_DataN_Count`, `_Resolution`, `_Time`.
- **Reading a data input's struct**: declare your own matching struct and index the injected buffer directly (`MyRec r = _Data0[i];`). Keep the field order byte-identical to the manifest schema on both the producer and consumer side.
